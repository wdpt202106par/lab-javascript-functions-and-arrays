...
// Bonus Calculate the sum / TOFINISH
...
// bonus Iteration #4.1: A generic avg() function / TOFINISH
function avg(arr) {}
...
function doesWordExist(arrayWord, word) {
  for (i = 0; i < arrayWord.length; i++) {
    if (arrayWord.includes(word) === true) {
      return true;
    } else {
      return false;
    }
...
function howManyTimes(array, wordInput) {
  let result = 0;
  for (i = 0; i < array.length; i++) {
    if (wordInput === array[i]) {
      result = result + 1;
    } else {
    }
  }
